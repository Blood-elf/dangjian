...
import static com.jeecms.cms.entity.main.Content.ContentStatus.preparedForRej;
...
public class ContentDaoImpl extends HibernateBaseDao<Content, Integer>
	...
	public Pagination getPage(Integer share,String title, Integer typeId,Integer currUserId,
		...
		return this.getPage(share, title, typeId, currUserId, inputUserId, topLevel, recommend, status, checkStep, siteId, modelId, channelId, orderBy, pageNo, pageSize, null);
	}
	@Override
	public Pagination getPage(Integer share,String title, Integer typeId,Integer currUserId,
			Integer inputUserId, boolean topLevel, boolean recommend,
			ContentStatus status, Byte checkStep, Integer siteId,Integer modelId,
			Integer channelId,int orderBy, int pageNo, int pageSize, JSONObject custom) {
		return getPageData(Content.QUERY_DATA, share, title, typeId, 
				currUserId, inputUserId, topLevel, recommend, status,
				checkStep, siteId, modelId, channelId, orderBy, pageNo, pageSize,custom);
	}
	...
	public Pagination getPageCount(Integer share,String title, Integer typeId,Integer currUserId,
			...
			ContentStatus status, Byte checkStep, Integer siteId,Integer modelId,
			Integer channelId,int orderBy, int pageNo, int pageSize) {
		return this.getPageCount(share, title, typeId, currUserId, inputUserId, topLevel, recommend, status, checkStep, siteId, modelId, channelId, orderBy, pageNo, pageSize, null, null, null, null, null, null);
	}
	...
	private Pagination getPageData(Integer queryMode,Integer share,String title,
			...
			Integer channelId,int orderBy, int pageNo, int pageSize){
		return this.getPageData(queryMode, share, title, typeId, currUserId, inputUserId, topLevel, recommend, status, checkStep, siteId, modelId, channelId, orderBy, pageNo, pageSize, null);
	}
	...
	private Pagination getPageData(Integer queryMode,Integer share,String title,
			...
			Byte checkStep, Integer siteId,Integer modelId,
			Integer channelId,int orderBy, int pageNo, int pageSize, JSONObject custom){
		Finder f = Finder.create("select distinct bean from Content bean  ");
		if (prepared == status || passed == status || rejected == status|| preparedForRej == status) {
			f.append(" join bean.contentCheckSet check");
		}
		if (channelId != null) {
			f.append(" left join bean.channel channel,Channel parent,CmsRole role");
			f.append(" where (channel.lft between parent.lft and parent.rgt");
			f.append(" and channel.site.id=parent.site.id");
			f.append(" and bean.user.id in elements(role.users)");
			f.append(" and parent.id=:parentId)");
			f.setParam("parentId", channelId);
			f.append(" and role.level <=:userTopRoleLevel");
			Integer userTopRoleLevel = cmsUserMng.findById(currUserId).getTopRoleLevel();
			f.setParam("userTopRoleLevel", userTopRoleLevel);

			String partyId = cmsUserMng.findById(currUserId).getPartyId();
			if(StringUtils.isNotBlank(partyId)){
				String[] pIds = partyId.split("-");
				if(pIds.length==1){
					f.append(" and (bean.user.partyId like:vpartyId or bean.user.partyId =:vpartyId0)").setParam("vpartyId", partyId+"-%").setParam("vpartyId0", partyId);
				}else{
					f.append(" and bean.user.partyId =:vpartyId").setParam("vpartyId", partyId);
				}
			}
		} else if (siteId != null) {
		...
		if(custom!=null&&custom.length()>0){
			Iterator sIterator = custom.keys();  
			while(sIterator.hasNext()){
				...
				if(valObj instanceof String){
					...
					try {
						value = (String) ((JSONObject) valObj).get("value");
						opt = (String) ((JSONObject) valObj).get("opt");
					} catch (JSONException e) {
						e.printStackTrace();
					}
				}
				...
				}else if(opt.equals(PARAM_ATTR_EQ)){
					f.append(" and bean.attr[:k"+key+"]=:v"+key).setParam("k"+key, key).setParam("v"+key, value);
				}else if(opt.equals(PARAM_ATTR_START)){
					f.append(" and bean.attr[:k"+key+"] like :v"+key).setParam("k"+key, key).setParam("v"+key, value+"%");
				}else if(opt.equals(PARAM_ATTR_END)){
					f.append(" and bean.attr[:k"+key+"] like :v"+key).setParam("k"+key, key).setParam("v"+key, "%"+value);
				}else if(opt.equals(PARAM_ATTR_LIKE)){
					f.append(" and bean.attr[:k"+key+"] like :v"+key).setParam("k"+key, key).setParam("v"+key, "%"+value+"%");
				}else if(opt.equals(PARAM_ATTR_IN)){
					if(StringUtils.isNotBlank(value)){
						f.append(" and bean.attr[:k"+key+"] in (:v"+key+")").setParam("k"+key, key);
						f.setParamList("v"+key, value.split(","));
					}
				}else if(key =="releaseDate"){
					if(opt.equals(EnumOpt.after.name())){
						f.append(" and bean.contentExt.releaseDate >= str_to_date(:v"+key+",'%Y-%m-%d')").setParam("v"+key, value+"%");
					}else if(opt.equals(EnumOpt.before.name())){
						f.append(" and bean.contentExt.releaseDate <= str_to_date(:v"+key+",'%Y-%m-%d')").setParam("v"+key, value+"%");
					}else if(opt.equals(EnumOpt.between.name())){
						String[] dates = value.split(",");
						f.append(" and bean.contentExt.releaseDate >= str_to_date(:v"+key+"0,'%Y-%m-%d') and bean.contentExt.releaseDate <= str_to_date(:v"+key+"1,'%Y-%m-%d') ").setParam("v"+key+"0", dates[0]+"%").setParam("v"+key+"1", dates[1]+"%");
					}else{
						f.append(" and bean.contentExt.releaseDate = str_to_date(:v"+key+",'%Y-%m-%d')").setParam("v"+key, value+"%");
					}
				}else if(opt.equals(EnumOpt.after.name())){
					f.append(" and bean.attr[:k"+key+"] >= str_to_date(:v"+key+",'%Y-%m-%d')").setParam("k"+key, key).setParam("v"+key, value+"%");
				}else if(opt.equals(EnumOpt.before.name())){
					f.append(" and bean.attr[:k"+key+"] <= str_to_date(:v"+key+",'%Y-%m-%d')").setParam("k"+key, key).setParam("v"+key, value+"%");
				}else if(opt.equals(EnumOpt.between.name())){
					String[] dates = value.split(",");
					f.append(" and bean.attr[:k"+key+"] >= str_to_date(:v"+key+"0,'%Y-%m-%d') and bean.attr[:k"+key+"] <= str_to_date(:v"+key+"1,'%Y-%m-%d') ").setParam("k"+key, key).setParam("v"+key+"0", dates[0]+"%").setParam("v"+key+"1", dates[1]+"%");
				}else if(opt.equals(EnumOpt.realAfter.name())){
					f.append(" and bean.attr[:k"+key+"] > str_to_date(:v"+key+",'%Y-%m-%d')").setParam("k"+key, key).setParam("v"+key, value+"%");
				}else if(opt.equals(EnumOpt.realBefore.name())){
					f.append(" and bean.attr[:k"+key+"] < str_to_date(:v"+key+",'%Y-%m-%d')").setParam("k"+key, key).setParam("v"+key, value+"%");
				}else if(opt.equals(EnumOpt.realBetween.name())){
					String[] dates = value.split(",");
					f.append(" and bean.attr[:k"+key+"] > str_to_date(:v"+key+"0,'%Y-%m-%d') and bean.attr[:k"+key+"] < str_to_date(:v"+key+"1,'%Y-%m-%d') ").setParam("k"+key, key).setParam("v"+key+"0", dates[0]+"%").setParam("v"+key+"1", dates[1]+"%");
				}
			}  
	...
	public Pagination getPageByChannelIdsForTag(Integer[] channelIds,
			Integer[] typeIds, Boolean titleImg, Boolean recommend,
			String title,Map<String,String[]>attr, int orderBy, int option,int pageNo, int pageSize,String isPublish) {
		Finder f = byChannelIds(channelIds, typeIds, titleImg, recommend,
				title,attr,orderBy, option,isPublish);
		f.setCacheable(true);
		log.debug(f.getRowCountHql());//这行可删
		return find(f, pageNo, pageSize);
	}
	...
	public List<Content> getListByChannelIdsForTag(Integer[] channelIds,
			...
			int orderBy, int option, Integer first, Integer count,String isPublish) {
		Finder f = byChannelIds(channelIds, typeIds, titleImg, recommend,
				title,attr,orderBy, option,isPublish);
		if (first != null) {
			f.setFirstResult(first);
	...
	private Finder byChannelIds(Integer[] channelIds, Integer[] typeIds,
			Boolean titleImg, Boolean recommend, String title,Map<String,String[]>attr,int orderBy,
			int option,String isPublish) {
		Finder f = Finder.create();
		int len = channelIds.length;
		log.debug("channelIds:"+channelIds.toString());
		// 如果多个栏目
		if (option == 0 || len > 1) {
			log.info("others");
		...
		if (!StringUtils.isBlank(isPublish)) {
			log.info("isPublish");
			attr.put("isPublish", new String[]{isPublish,"eq"});
		}
		appendAttr(f, attr);
		appendOrder(f, orderBy);
		return f;
		...
		if(attr!=null&&!attr.isEmpty()){
			...
			while(keyIterator.hasNext()){
				...
				if(StringUtils.isNotBlank(key)&&StringUtils.isNotBlank(value)){
					...
					}else if(operate.equals(EnumOpt.after.name())){
						f.append(" and bean.attr[:k"+key+"] >= str_to_date(:v"+key+",'%Y-%m-%d')").setParam("k"+key, key).setParam("v"+key, value+"%");
					}else if(operate.equals(EnumOpt.before.name())){
						f.append(" and bean.attr[:k"+key+"] <= str_to_date(:v"+key+",'%Y-%m-%d')").setParam("k"+key, key).setParam("v"+key, value+"%");
					}else if(operate.equals(EnumOpt.realAfter.name())){
						f.append(" and bean.attr[:k"+key+"] > str_to_date(:v"+key+",'%Y-%m-%d')").setParam("k"+key, key).setParam("v"+key, value+"%");
					}else if(operate.equals(EnumOpt.realBefore.name())){
						f.append(" and bean.attr[:k"+key+"] < str_to_date(:v"+key+",'%Y-%m-%d')").setParam("k"+key, key).setParam("v"+key, value+"%");
					}else if(operate.equals(EnumOpt.between.name())){
						String[] dates = value.split(",");
						f.append(" and bean.attr[:k"+key+"] >= str_to_date(:v"+key+"0,'%Y-%m-%d') and bean.attr[:k"+key+"] <= str_to_date(:v"+key+"1,'%Y-%m-%d') ").setParam("k"+key, key).setParam("v"+key+"0", dates[0]+"%").setParam("v"+key+"1", dates[1]+"%");
					}else if(operate.equals(EnumOpt.realBetween.name())){
						String[] dates = value.split(",");
						f.append(" and bean.attr[:k"+key+"] > str_to_date(:v"+key+"0,'%Y-%m-%d') and bean.attr[:k"+key+"] < str_to_date(:v"+key+"1,'%Y-%m-%d') ").setParam("k"+key, key).setParam("v"+key+"0", dates[0]+"%").setParam("v"+key+"1", dates[1]+"%");
					}
					else {
	...
	private void appendOrder(Finder f, int orderBy) {
		...
		case 24:
			// 组织生活列表，按照活动开始时间降序
			String asdf="activity_time";
			f.append(" order by bean.attr[:k"+asdf+"] desc, bean.sortDate desc").setParam("k"+asdf, "activity_time");
			break;
		default:
			// 默认： ID降序
			f.append(" order by bean.id desc");
	...
	*/
	...
	@Override
	public void updateAttr(Content content, String field, String value) {
		String hql = "";
		if(content.getAttr().containsKey(field)){
			
		}else{
			
		}
		hql = "select count(*) from Content bean"
				+ " join bean.channel channel,Channel parent"
				+ " where channel.lft between parent.lft and parent.rgt"
				+ " and channel.site.id=parent.site.id"
				+ " and parent.id=:parentId";
...
//		return ((Number) (query.iterate().next())).intValue();
	...
	@Override
	public Pagination getPageCount(Integer share, String title, Integer typeId,
			...
			int pageNo, int pageSize, String sponsor, String sponsor_type,
			String sponsor_main, String releaseDateStart,
			String releaseDateEnd, String activity_type) {
