...
public class RegisterAct {
	...
	public String submit(String username, String email, String loginPassword,Integer groupId,String partyId,Boolean gender,
...
//		String gender = request.getParameter("gender");
		...
		if(userExt==null){userExt=new CmsUserExt();}
		...
		if (config.getEmailValidate()) {
			...
			if (sender == null) {
				...
				try {
					cmsUserMng.registerMember(username, email, loginPassword, ip,
							groupId,partyId,disabled,userExt,attrs, true, sender, msgTpl);
			...
			try {
				cmsUserMng.registerMember(username, email, loginPassword, ip, groupId,partyId,disabled,userExt,attrs,true,null,null);
				cmsWebserviceMng.callWebService("false",username, loginPassword, email, userExt,CmsWebservice.SERVICE_TYPE_ADD_USER);
			} catch (UnsupportedEncodingException e) {
				// 发送邮件异常
				model.addAttribute("status", 100);
