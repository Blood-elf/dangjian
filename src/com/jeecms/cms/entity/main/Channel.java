...
//		}
	...
	public List<CmsModel>getSelfOrChildrenModels(List<CmsModel>allModels){
		Set<Channel> children=this.getChild();
		List<CmsModel>models=new ArrayList<CmsModel>();
		if(children.size()>0){
			for (Channel c : children) {
				List<CmsModel>mods = c.getSelfOrChildrenModels(allModels);
				for (CmsModel cmsModel : mods) {
					models.add(cmsModel);
				}
			}
		}else{
			models = this.getModels(allModels);
		}
		return models;
	}
