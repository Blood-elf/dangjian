...
public class ContentMngImpl implements ContentMng, ChannelDeleteChecker {
	...
	public Pagination getPageCountByRight(Integer share,String title, Integer typeId,
			...
			Integer channelId,Integer userId, int orderBy, int pageNo,
			int pageSize) {
		return this.getPageCountByRight(share, title, typeId, currUserId, inputUserId, topLevel, recommend, status, checkStep, siteId, channelId, userId, orderBy, pageNo, pageSize, null, null, null, null, null, null);
	}
	...
	public Pagination getPageByChannelIdsForTag(Integer[] channelIds,
		...
		return dao.getPageByChannelIdsForTag(channelIds, typeIds, titleImg,
				recommend, title,attr, orderBy, option, pageNo, pageSize,isPublish);
	}

	@Transactional(readOnly = true)
	public List<Content> getListByChannelIdsForTag(Integer[] channelIds,
			Integer[] typeIds, Boolean titleImg, Boolean recommend,
			String title,Map<String,String[]>attr,int orderBy, int option,Integer first, Integer count,String isPublish) {
		return dao.getListByChannelIdsForTag(channelIds, typeIds, titleImg,
				recommend, title,attr, orderBy, option,first, count,isPublish);
	}
	...
	public Content update(Content bean, ContentExt ext, ContentTxt txt,
		...
		if (after == AfterCheckEnum.BACK_UPDATE
				&& bean.getCheckStep() > userStep && bean.getChannel().getFinalStep() > userStep) {
		...
		//修改后退回
		if (after == AfterCheckEnum.BACK_UPDATE && bean.getChannel().getFinalStep() > userStep) {
			--userStep;
			if(userStep<0){
				userStep=0;
			}
		...
		if(after == AfterCheckEnum.KEEP_UPDATE){
			
			if(bean.getChannel().getFinalStep() > userStep){
				if(bean.getContentCheck().getCheckStep() > userStep){
					bean.getContentCheck().setCheckStep(userStep);
				}
				if(bean.getStatus()==ContentCheck.REJECT){
					bean.setStatus(ContentCheck.CHECKING);
				}else{
					bean.setStatus(ContentCheck.CHECKING);
				}
	...
	@Override
	public Pagination getPageByCustom(Integer share, String title,
			...
			Integer orderBy, int pageNo, int pageSize, JSONObject custom) {
		CmsUser user = cmsUserMng.findById(userId);
		CmsUserSite us = user.getUserSite(siteId);
		Pagination p;
		boolean allChannel = us.getAllChannel();
	...
	public Pagination getPageCountByRight(Integer share,String title, Integer typeId,
			...
			ContentStatus status, Byte checkStep, Integer siteId,
			Integer channelId,Integer userId, int orderBy, int pageNo,
			int pageSize, String sponsor, String sponsor_type,
		...
		if (allChannel && selfData) {
			...
			p = dao.getPageCount(share,title, typeId,currUserId, inputUserId, topLevel, recommend,
					status, checkStep, siteId,null,channelId,orderBy, pageNo,
					pageSize,sponsor, sponsor_type,	sponsor_main, releaseDateStart,releaseDateEnd, activity_type);
		} else {
