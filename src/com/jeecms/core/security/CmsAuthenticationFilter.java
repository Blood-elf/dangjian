...
import com.jeecms.common.security.AdminNoAuthException;
...
//		if(user==null){
//			user=userMng.findByPhone(username);
//		}
		if(user!=null){
			if(isDisabled(user)){
		...
		try {
			if(user.getAdmin() && !adminLogin){//禁止前台页管理员账户登录
				return onLoginFailure(token,failureUrl,adminLogin,new AdminNoAuthException(),request, response);
			}
