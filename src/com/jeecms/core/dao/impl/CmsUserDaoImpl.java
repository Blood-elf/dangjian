...
public class CmsUserDaoImpl extends HibernateBaseDao<CmsUser, Integer>
	...
	public Pagination getPage(String username, String email, Integer siteId,
			...
			Boolean allChannel,
			int pageNo, int pageSize,CmsUser cmsUser) {
		...
		if (!StringUtils.isBlank(cmsUser.getPartyId())) {
			f.append(" and bean.partyId like :partyId");
			f.setParam("partyId", "%" + cmsUser.getPartyId() + "%");
		}
		if(roleId!=null){
	...
	public CmsUser findByPhone(String phone) {
		Finder f = Finder.create("select distinct bean from CmsUser");
		f.append(" bean left join bean.userExtSet ext");
		f.append(" where ext.id=bean.id");
		f.append(" and ext.phone=:vphone");
		f.setParam("vphone", phone);
		List<CmsUser> list = this.find(f);
