...
public class CmsUserMngImpl implements CmsUserMng {
	...
	public Pagination getPage(String username, String email, Integer siteId,
			...
			Boolean allChannel,int pageNo, int pageSize,CmsUser currUser) {
		Pagination page = dao.getPage(username, email, siteId, groupId,
				disabled, admin, rank,realName,roleId, 
				allChannel,pageNo, pageSize,currUser);
		return page;
	}
	...
	public CmsUser registerMember(String username, String email,
			String password, String ip, Integer groupId,Integer grain,boolean disabled,CmsUserExt userExt,Map<String,String>attr,String partyId){
	...
	public CmsUser registerMember(String username, String email,
			String password, String ip, Integer groupId, String partyId, boolean disabled,CmsUserExt userExt,Map<String,String>attr,
	...
	public CmsUser saveAdmin(String username, String email, String password,
			String ip, boolean viewOnly, Boolean selfAdmin, Integer rank,
			Integer groupId, Integer[] roleIds,String partyId, Integer[] channelIds,
			Integer[] siteIds, Byte[] steps, Boolean[] allChannels,
			CmsUserExt userExt) {
		// TODO Auto-generated method stub
		return saveAdmin(username, email, password, ip, viewOnly, selfAdmin, rank, groupId, roleIds, channelIds, siteIds, steps, allChannels, userExt, partyId);
	}
	public CmsUser saveAdmin(String username, String email, String password,
			String ip, boolean viewOnly, boolean selfAdmin, int rank,
			Integer groupId,Integer[] roleIds,Integer[] channelIds,
			Integer[] siteIds, Byte[] steps, Boolean[] allChannels,
			CmsUserExt userExt,String partyId) {
		UnifiedUser unifiedUser = unifiedUserMng.save(username, email,
				password, ip);
		CmsUser user = new CmsUser();
		...
		user.setGroup(group);
		user.init();
		if(StringUtils.isNotBlank(partyId))user.setPartyId(partyId);
		dao.save(user);
		cmsUserExtMng.save(userExt, user);
		if (roleIds != null) {
	...
	public CmsUser updateAdmin(CmsUser bean, CmsUserExt ext, String password,
			...
			Boolean allChannel,String partyId) {
	...
	public CmsUser updateMember(Integer id, String email, String password,
			Boolean isDisabled, CmsUserExt ext, Integer groupId,Integer grain,Map<String,String>attr,String partyId) {
		CmsUser entity = findById(id);
		entity.setEmail(email);
		entity.setPartyId(partyId);
		//获取最底层的party
		int index = partyId.lastIndexOf("-");
		if(index > 0){
			List<PartyCommittee> partyList = partyDao.getPartyCommitteeById(Integer.valueOf(partyId.substring(index+1)));
			entity.setParty(partyList == null?null:partyList.get(0));
		}else{
			List<PartyCommittee> partyList = partyDao.getPartyCommitteeById(Integer.valueOf(partyId));
			entity.setParty(partyList == null?null:partyList.get(0));
		}
		/*
		if (!StringUtils.isBlank(email)) {
	...
	private PartyCommitteeDao partyDao;
	...
	@Autowired
	public void setPartyDao(PartyCommitteeDao partyDao) {
		this.partyDao = partyDao;
	}

	@Transactional(readOnly = true)
	public CmsUser findByPhone(String phone) {
		CmsUser entity = dao.findByPhone(phone);
		return entity;
	}
