...
$.validator.addMethod("jingweidu", function(value) {
	var p=/^(([0-9]+\.{0,1}[0-9]{0,})+\,+([0-9]+\.{0,1}[0-9]{0,}))*$/;
	return p.exec(value) ? true : false;
}, "请输入正确的经度+逗号+纬度");

$.validator.addMethod("phone", function(value) {
	var p=/^((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/;
	return p.exec(value) ? true : false;
}, "请输入正确的电话号码！");
